...
///     MAME's aviio reader, used to benchmark the 'avhu' (laserdisc) encode path of
///     <c>ChdEncoder.EncodeLaserDisc</c> and chdman's <c>createld</c>. Same minimal layout the
///     test-corpus generator (<c>CHDSharpTestDataGeneration</c>) uses: RIFF/AVI with hdrl
///     (avih + two strl streams), movi chunks ('00dc' video + '01wb' audio) and an idx1 index.
/// </summary>
internal static class SyntheticAvi
{
    /// <summary>
    ///     Writes a deterministic AVI: a moving bright bar over a smooth gradient video frame
    ///     plus a 440 Hz-ish sine audio track (compresses realistically for delta-RLE video).
    /// </summary>
    /// <param name="path">Output AVI path.</param>
    /// <param name="width">Video width in pixels (YUY2: 2 bytes per pixel).</param>
    /// <param name="height">Video height in pixels.</param>
    /// <param name="fps">Frame rate; <paramref name="sampleRate" /> must be divisible by it.</param>
    /// <param name="frames">Number of video frames (one laserdisc hunk each).</param>

...
public static unsafe partial class Methods
{
    ...
    )
    {
        ...
        switch (flSize)
        {
            ...
            case 2:
                MEM_writeLE16(
                    ostart,
                    (ushort)((uint)SymbolEncodingTypeE.SetBasic + (1 << 2) + (srcSize << 4))
                );
                break;
            case 3:
                MEM_writeLE32(
                    ostart,
                    (uint)((uint)SymbolEncodingTypeE.SetBasic + (3 << 2) + (srcSize << 4))
                );
                break;
            default:
                assert(0 != 0);
    ...
    )
    {
        ...
        switch (flSize)
        {
            ...
            case 2:
                MEM_writeLE16(
                    ostart,
                    (ushort)((uint)SymbolEncodingTypeE.SetRle + (1 << 2) + (srcSize << 4))
                );
                break;
            case 3:
                MEM_writeLE32(
                    ostart,
                    (uint)((uint)SymbolEncodingTypeE.SetRle + (3 << 2) + (srcSize << 4))
                );
                break;
            default:
                assert(0 != 0);
    ...
    )
    {
...

        {
            ...
            var flags =
                0
                | (bmi2 != 0 ? (int)HufFlagsE.HufFlagsBmi2 : 0)
                | (
                    strategy < ZstdStrategy.ZstdLazy && srcSize <= 1024
                        ? (int)HufFlagsE.HufFlagsPreferRepeat
                        : 0
                )
                | (
                    strategy >= ZstdStrategy.ZstdBtultra
                        ? (int)HufFlagsE.HufFlagsOptimalDepth
                        : 0
                )
                | (
                    suspectUncompressible != 0
                        ? (int)HufFlagsE.HufFlagsSuspectUncompressible
                        : 0
                );
            void* hufCompress;

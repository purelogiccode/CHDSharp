...
public static unsafe partial class Methods
{
    ...
    private static nuint HUF_compressWeights(void* dst, nuint dstSize, void* weightTable, nuint wtSize, void* workspace, nuint workspaceSize)
    {
        ...
        {
            var hSize = FSE_writeNCount(op, (nuint)(oend - op), wksp->norm, maxSymbolValue, tableLog);
        ...
        {
            var cSize = FSE_compress_usingCTable(op, (nuint)(oend - op), weightTable, wtSize, wksp->CTable);
    ...
    private static int HUF_buildTree(nodeElt_s* huffNode, uint maxSymbolValue)
    {
        ...
        var nodeNb = 255 + 1;
        int n;
        var nonNullRank = (int)maxSymbolValue;
    ...
    private static nuint HUF_compress_internal(void* dst, nuint dstSize, void* src, nuint srcSize, uint maxSymbolValue, uint huffLog, HUF_nbStreams_e nbStreams, void* workSpace, nuint wkspSize, nuint* oldHufTable, HUF_repeat* repeat, int flags)
    {
        ...
        {
            var largest = HIST_count_wksp(table->count, &maxSymbolValue, (byte*)src, srcSize, table->wksps.hist_wksp, sizeof(uint) * 1024);

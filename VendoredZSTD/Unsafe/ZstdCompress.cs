...
public static unsafe partial class Methods
{
    ...
    private static void ZSTD_initCCtx(ZSTD_CCtx_s* cctx, ZSTD_customMem memManager)
    {
        ...
        *cctx = new ZSTD_CCtx_s
        ...
        {
            var err = ZSTD_CCtx_reset(cctx, ZSTD_ResetDirective.ZSTD_reset_parameters);
    ...
    public static ZSTD_CCtx_s* ZSTD_initStaticCCtx(void* workspace, nuint workspaceSize)
    {
        ...
        var cctx = (ZSTD_CCtx_s*)ZSTD_cwksp_reserve_object(&ws, (nuint)sizeof(ZSTD_CCtx_s));
        if (cctx == null)
            return null;
    ...
    {
        return strategy is >= ZSTD_strategy.ZSTD_greedy and <= ZSTD_strategy.ZSTD_lazy2 ? 1 : 0;
    }
    ...
    private static ZSTD_CCtx_params_s* ZSTD_createCCtxParams_advanced(ZSTD_customMem customMem)
    {
        ...
        var @params = (ZSTD_CCtx_params_s*)ZSTD_customCalloc((nuint)sizeof(ZSTD_CCtx_params_s), customMem);
        if (@params == null)
        {
            return null;
        }
     ...
     */
    public static ZSTD_bounds ZSTD_cParam_getBounds(ZstdCParameter param)
    {
     ...
     */
    public static nuint ZSTD_CCtx_setParameter(ZSTD_CCtx_s* cctx, ZstdCParameter param, int value)
    {
     ...
     *           ZSTD_isError()).
     */
    public static nuint ZSTD_CCtxParams_setParameter(ZSTD_CCtx_params_s* CCtxParams, ZstdCParameter param, int value)
    {
        switch (param)
        {
            case ZstdCParameter.ZstdCExperimentalParam2:
            ...
            case ZstdCParameter.ZstdCExperimentalParam4:
            {
                var pref = (ZSTD_dictAttachPref_e)value;
            ...
            case ZstdCParameter.ZstdCExperimentalParam5:
            {
                var lcm = (ZSTD_paramSwitch_e)value;
            ...
            case ZstdCParameter.ZstdCOverlapLog:
            {
                var err_code = ZSTD_cParam_clampBounds(ZstdCParameter.ZstdCOverlapLog, &value);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
     ...
     */
    public static nuint ZSTD_CCtx_getParameter(ZSTD_CCtx_s* cctx, ZstdCParameter param, int* value)
    {
     ...
     */
    public static nuint ZSTD_CCtxParams_getParameter(ZSTD_CCtx_params_s* CCtxParams, ZstdCParameter param, int* value)
    {
        switch (param)
        {
            case ZstdCParameter.ZstdCExperimentalParam2:
    ...
    public static nuint ZSTD_CCtx_setCParams(ZSTD_CCtx_s* cctx, ZSTD_compressionParameters cparams)
    {
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCWindowLog, (int)cparams.windowLog);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCChainLog, (int)cparams.chainLog);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCHashLog, (int)cparams.hashLog);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCSearchLog, (int)cparams.searchLog);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCMinMatch, (int)cparams.minMatch);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCTargetLength, (int)cparams.targetLength);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCStrategy, (int)cparams.strategy);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
     ...
     * @return 0 on success, or an error code (can be checked with ZSTD_isError()).
     */
    public static nuint ZSTD_CCtx_setFParams(ZSTD_CCtx_s* cctx, ZstdFrameParameters fparams)
    {
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCContentSizeFlag, fparams.contentSizeFlag != 0 ? 1 : 0);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCChecksumFlag, fparams.checksumFlag != 0 ? 1 : 0);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(cctx, ZstdCParameter.ZstdCDictIdFlag, fparams.noDictIDFlag == 0 ? 1 : 0);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    {
        if (reset is ZSTD_ResetDirective.ZSTD_reset_session_only or ZSTD_ResetDirective.ZSTD_reset_session_and_parameters)
        {
            cctx->streamStage = ZSTD_cStreamStage.zcss_init;
            cctx->pledgedSrcSizePlusOne = 0;
        }

        if (reset is ZSTD_ResetDirective.ZSTD_reset_parameters or ZSTD_ResetDirective.ZSTD_reset_session_and_parameters)
        {
            if (cctx->streamStage != ZSTD_cStreamStage.zcss_init)
            {
                return unchecked((nuint)(-(int)ZSTD_ErrorCode.ZSTD_error_stage_wrong));
    ...
    {
        var btScale = (uint)strat >= (uint)ZSTD_strategy.ZSTD_btlazy2 ? 1U : 0U;
    ...
    private static nuint ZSTD_sizeof_matchState(ZSTD_compressionParameters* cParams, ZSTD_paramSwitch_e useRowMatchFinder, int enableDedicatedDictSearch, uint forCCtx)
    {
        ...
        var optSpace = forCCtx != 0 && cParams->strategy >= ZSTD_strategy.ZSTD_btopt ? optPotentialSpace : 0;
    ...
    private static nuint ZSTD_estimateCCtxSize_usingCCtxParams_internal(ZSTD_compressionParameters* cParams, ldmParams_t* ldmParams, int isStatic, ZSTD_paramSwitch_e useRowMatchFinder, nuint buffInSize, nuint buffOutSize, ulong pledgedSrcSize, int useSequenceProducer, nuint maxBlockSize)
    {
        ...
        var tokenSpace = ZSTD_cwksp_alloc_size(32 + blockSize) + ZSTD_cwksp_aligned64_alloc_size(maxNbSeq * (nuint)sizeof(SeqDef_s)) + 3 * ZSTD_cwksp_alloc_size(maxNbSeq * sizeof(byte));
        ...
        var ldmSeqSpace = ldmParams->enableLdm == ZSTD_paramSwitch_e.ZSTD_ps_enable ? ZSTD_cwksp_aligned64_alloc_size(maxNbLdmSeq * (nuint)sizeof(rawSeq)) : 0;
        ...
        var externalSeqSpace = useSequenceProducer != 0 ? ZSTD_cwksp_aligned64_alloc_size(maxNbExternalSeq * (nuint)sizeof(ZSTD_Sequence)) : 0;
    ...
    {
        var cParams = ZSTD_getCParamsFromCCtxParams(@params, unchecked(0UL - 1), 0, ZSTD_CParamMode_e.ZSTD_cpm_noAttachDict);
    ...
    private static nuint ZSTD_estimateCCtxSize_internal(int compressionLevel)
    {
        ...
        for (; tier < 4; ++tier)
        {
            ...
            var cParams = ZSTD_getCParams_internal(compressionLevel, srcSizeTiers[tier], 0, ZSTD_CParamMode_e.ZSTD_cpm_noAttachDict);
    ...
    public static nuint ZSTD_estimateCStreamSize_usingCCtxParams(ZSTD_CCtx_params_s* @params)
    {
...

        {
            ...
            var inBuffSize = @params->inBufferMode == ZSTD_bufferMode_e.ZSTD_bm_buffered ? ((nuint)1 << (int)cParams.windowLog) + blockSize : 0;
            var outBuffSize = @params->outBufferMode == ZSTD_bufferMode_e.ZSTD_bm_buffered ? ZSTD_compressBound(blockSize) + 1 : 0;
    ...
    {
        var cParams = ZSTD_getCParams_internal(compressionLevel, unchecked(0UL - 1), 0, ZSTD_CParamMode_e.ZSTD_cpm_noAttachDict);
    ...
    private static nuint ZSTD_reset_matchState(ZSTD_MatchState_t* ms, ZstdCwksp* ws, ZSTD_compressionParameters* cParams, ZSTD_paramSwitch_e useRowMatchFinder, ZSTD_compResetPolicy_e crp, ZSTD_indexResetPolicy_e forceResetIndex, ZSTD_resetTarget_e forWho)
    {
        /* disable chain table allocation for fast or row-based strategies */
        var chainSize = ZSTD_allocateChainTable(cParams->strategy, useRowMatchFinder, ms->dedicatedDictSearch != 0 && forWho == ZSTD_resetTarget_e.ZSTD_resetTarget_CDict ? 1U : 0U) != 0 ? (nuint)1 << (int)cParams->chainLog : 0;
        var hSize = (nuint)1 << (int)cParams->hashLog;
        var hashLog3 = forWho == ZSTD_resetTarget_e.ZSTD_resetTarget_CCtx && cParams->minMatch == 3 ? 17 < cParams->windowLog ? 17 : cParams->windowLog : 0;
        var h3Size = hashLog3 != 0 ? (nuint)1 << (int)hashLog3 : 0;
        assert(useRowMatchFinder != ZSTD_paramSwitch_e.ZSTD_ps_auto);
    ...
    private static nuint ZSTD_resetCCtx_internal(ZSTD_CCtx_s* zc, ZSTD_CCtx_params_s* @params, ulong pledgedSrcSize, nuint loadedDictSize, ZSTD_compResetPolicy_e crp, ZSTD_buffered_policy_e zbuff)
    {
...

        {
            ...
            var buffOutSize = zbuff == ZSTD_buffered_policy_e.ZSTDb_buffered && @params->outBufferMode == ZSTD_bufferMode_e.ZSTD_bm_buffered ? ZSTD_compressBound(blockSize) + 1 : 0;
            var buffInSize = zbuff == ZSTD_buffered_policy_e.ZSTDb_buffered && @params->inBufferMode == ZSTD_bufferMode_e.ZSTD_bm_buffered ? windowSize + blockSize : 0;
            ...
            var needsIndexReset = indexTooClose != 0 || dictTooBig != 0 || zc->initialized == 0 ? ZSTD_indexResetPolicy_e.ZSTDirp_reset : ZSTD_indexResetPolicy_e.ZSTDirp_continue;
            ...
            {
                var err_code = ZSTD_reset_matchState(&zc->blockState.matchState, ws, &@params->cParams, @params->useRowMatchFinder, crp, needsIndexReset, ZSTD_resetTarget_e.ZSTD_resetTarget_CCtx);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
    ...
    {
        {
            var adjusted_cdict_cParams = cdict->matchState.cParams;
            ...
            {
                var err_code = ZSTD_resetCCtx_internal(cctx, &@params, pledgedSrcSize, 0, ZSTD_compResetPolicy_e.ZSTDcrp_makeClean, zbuff);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
    ...
    {
        var cdict_cParams = &cdict->matchState.cParams;
        assert(cdict->matchState.dedicatedDictSearch == 0);
        {
            ...
            @params.useRowMatchFinder = cdict->useRowMatchFinder;
            {
                var err_code = ZSTD_resetCCtx_internal(cctx, &@params, pledgedSrcSize, 0, ZSTD_compResetPolicy_e.ZSTDcrp_leaveDirty, zbuff);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
        ...
        assert(@params.useRowMatchFinder != ZSTD_paramSwitch_e.ZSTD_ps_auto);
        {
            ...
            var chainSize = ZSTD_allocateChainTable(cdict_cParams->strategy, cdict->useRowMatchFinder, 0) != 0 ? (nuint)1 << (int)cdict_cParams->chainLog : 0;
    ...
    private static nuint ZSTD_copyCCtx_internal(ZSTD_CCtx_s* dstCCtx, ZSTD_CCtx_s* srcCCtx, ZstdFrameParameters fParams, ulong pledgedSrcSize, ZSTD_buffered_policy_e zbuff)
    {
    ...
    private static ZSTD_symbolEncodingTypeStats_t ZSTD_buildSequencesStatistics(SeqStore_t* seqStorePtr, nuint nbSeq, ZSTD_fseCTables_t* prevEntropy, ZSTD_fseCTables_t* nextEntropy, byte* dst, byte* dstEnd, ZSTD_strategy strategy, uint* countWorkspace, void* entropyWorkspace, nuint entropyWkspSize)
    {
        ...
        var oend = dstEnd;
        var op = ostart;
        var CTable_LitLength = nextEntropy->litlengthCTable;
        var CTable_OffsetBits = nextEntropy->offcodeCTable;
        var CTable_MatchLength = nextEntropy->matchlengthCTable;
        var ofCodeTable = seqStorePtr->ofCode;
        var llCodeTable = seqStorePtr->llCode;
        var mlCodeTable = seqStorePtr->mlCode;
        System.Runtime.CompilerServices.Unsafe.SkipInit(out ZSTD_symbolEncodingTypeStats_t stats);
        ...
        assert(nbSeq != 0);
        {
            ...
            assert(!(stats.LLtype < (uint)SymbolEncodingType_e.set_compressed && nextEntropy->litlength_repeatMode != FseRepeat.FseRepeatNone));
            {
                var countSize = ZSTD_buildCTable(op, (nuint)(oend - op), CTable_LitLength, 9, (SymbolEncodingType_e)stats.LLtype, countWorkspace, max, llCodeTable, nbSeq, LL_defaultNorm, LL_defaultNormLog, 35, prevEntropy->litlengthCTable, sizeof(uint) * 329, entropyWorkspace, entropyWkspSize);
...

        {
            ...
            var defaultPolicy = max <= 28 ? ZSTD_DefaultPolicy_e.ZSTD_defaultAllowed : ZSTD_DefaultPolicy_e.ZSTD_defaultDisallowed;
            ...
            assert(!(stats.Offtype < (uint)SymbolEncodingType_e.set_compressed && nextEntropy->offcode_repeatMode != FseRepeat.FseRepeatNone));
            {
                var countSize = ZSTD_buildCTable(op, (nuint)(oend - op), CTable_OffsetBits, 8, (SymbolEncodingType_e)stats.Offtype, countWorkspace, max, ofCodeTable, nbSeq, OF_defaultNorm, OF_defaultNormLog, 28, prevEntropy->offcodeCTable, sizeof(uint) * 193, entropyWorkspace, entropyWkspSize);
                if (ERR_isError(countSize))
                {
                    stats.size = countSize;
                    return stats;
...

        {
            ...
            assert(!(stats.MLtype < (uint)SymbolEncodingType_e.set_compressed && nextEntropy->matchlength_repeatMode != FseRepeat.FseRepeatNone));
            {
                var countSize = ZSTD_buildCTable(op, (nuint)(oend - op), CTable_MatchLength, 9, (SymbolEncodingType_e)stats.MLtype, countWorkspace, max, mlCodeTable, nbSeq, ML_defaultNorm, ML_defaultNormLog, 52, prevEntropy->matchlengthCTable, sizeof(uint) * 363, entropyWorkspace, entropyWkspSize);
                if (ERR_isError(countSize))
                {
                    stats.size = countSize;
                    return stats;
    ...
    private static nuint ZSTD_entropyCompressSeqStore_internal(void* dst, nuint dstCapacity, void* literals, nuint litSize, SeqStore_t* seqStorePtr, ZSTD_entropyCTables_t* prevEntropy, ZSTD_entropyCTables_t* nextEntropy, ZSTD_CCtx_params_s* cctxParams, void* entropyWorkspace, nuint entropyWkspSize, int bmi2)
    {
        ...
        var CTable_LitLength = nextEntropy->fse.litlengthCTable;
        var CTable_OffsetBits = nextEntropy->fse.offcodeCTable;
        var CTable_MatchLength = nextEntropy->fse.matchlengthCTable;
        var sequences = seqStorePtr->sequencesStart;
        ...
        {
            var bitstreamSize = ZSTD_encodeSequences(op, (nuint)(oend - op), CTable_MatchLength, mlCodeTable, CTable_OffsetBits, ofCodeTable, CTable_LitLength, llCodeTable, sequences, nbSeq, longOffsets, bmi2);
            {
...

        {
            ...
            else if (zc->appliedParams.ldmParams.enableLdm == ZSTD_paramSwitch_e.ZSTD_ps_enable)
            {
                ...
                ldmSeqStore.capacity = zc->maxNbLdmSequences;
                {
                    ...
                    var err_code = ZSTD_ldm_generateSequences(&zc->ldmState, &ldmSeqStore, &zc->appliedParams.ldmParams, src, srcSize);
                    if (ERR_isError(err_code))
                    {
                        return err_code;
                    }
            ...
            else if (ZSTD_hasExtSeqProd(&zc->appliedParams) != 0)
            {
                ...
                assert(zc->appliedParams.extSeqProdFunc != null);
                {
                    ...
                    var nbExternalSeqs = ((delegate* managed<void*, ZSTD_Sequence*, nuint, void*, nuint, void*, nuint, int, nuint, nuint>)zc->appliedParams.extSeqProdFunc)(zc->appliedParams.extSeqProdState, zc->extSeqBuf, zc->extSeqBufCapacity, src, srcSize, null, 0, zc->appliedParams.compressionLevel, windowSize);
                    ...
                    {
                        var seqPos = new ZSTD_SequencePosition
                        {
                            idx = 0,
                            posInSequence = 0,
                            posInSrc = 0
                        ...
                        {
                            var err_code = ZSTD_transferSequences_wBlockDelim(zc, &seqPos, zc->extSeqBuf, nbPostProcessedSeqs, src, srcSize, zc->appliedParams.searchForExternalRepcodes);
                            if (ERR_isError(err_code))
                            {
                                return err_code;
                            }
    ...
    public static nuint ZSTD_generateSequences(ZSTD_CCtx_s* zc, ZSTD_Sequence* outSeqs, nuint outSeqsSize, void* src, nuint srcSize)
    {
        ...
        SeqCollector seqCollector;
        {
            ...
            {
                var err_code = ZSTD_CCtx_getParameter(zc, ZstdCParameter.ZstdCTargetCBlockSize, &targetCBlockSize);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
...

        {
            int nbWorkers;
            {
                var err_code = ZSTD_CCtx_getParameter(zc, ZstdCParameter.ZstdCNbWorkers, &nbWorkers);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
    ...
    {
        var cBlockHeader = cSize == 1 ? lastBlock + ((uint)blockType_e.bt_rle << 1) + (uint)(blockSize << 3) : lastBlock + ((uint)blockType_e.bt_compressed << 1) + (uint)(cSize << 3);
    ...
    {
        var stats = new ZSTD_symbolEncodingTypeStats_t
        {
    ...
    private static nuint ZSTD_buildBlockEntropyStats(SeqStore_t* seqStorePtr, ZSTD_entropyCTables_t* prevEntropy, ZSTD_entropyCTables_t* nextEntropy, ZSTD_CCtx_params_s* cctxParams, ZSTD_entropyCTablesMetadata_t* entropyMetadata, void* workspace, nuint wkspSize)
    {
        ...
        var hufFlags = huf_useOptDepth != 0 ? (int)HufFlagsE.HufFlagsOptimalDepth : 0;
    ...
    private static nuint ZSTD_buildEntropyStatisticsAndEstimateSubBlockSize(SeqStore_t* seqStore, ZSTD_CCtx_s* zc)
    {
        ...
        {
            var err_code = ZSTD_buildBlockEntropyStats(seqStore, &zc->blockState.prevCBlock->entropy, &zc->blockState.nextCBlock->entropy, &zc->appliedParams, entropyMetadata, zc->tmpWorkspace, zc->tmpWkspSize);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    {
        uint idx = 0;
        var longLitLenIdx = seqStore->longLengthType == ZSTD_longLengthType_e.ZSTD_llt_literalLength ? seqStore->longLengthPos : nbSeq;
        for (; idx < nbSeq; ++idx)
        {
    ...
    private static nuint ZSTD_compressSeqStore_singleBlock(ZSTD_CCtx_s* zc, SeqStore_t* seqStore, repcodes_s* dRep, repcodes_s* cRep, void* dst, nuint dstCapacity, void* src, nuint srcSize, uint lastBlock, uint isPartition)
    {
        ...
        var cSeqsSize = ZSTD_entropyCompressSeqStore(seqStore, &zc->blockState.prevCBlock->entropy, &zc->blockState.nextCBlock->entropy, &zc->appliedParams, op + ZSTD_blockHeaderSize, dstCapacity - ZSTD_blockHeaderSize, srcSize, zc->tmpWorkspace, zc->tmpWkspSize, zc->bmi2);
        {
        ...
        {
            {
                var err_code = ZSTD_copyBlockSequences(&zc->seqCollector, seqStore, dRepOriginal.rep);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
    ...
    private static nuint ZSTD_compressBlock_internal(ZSTD_CCtx_s* zc, void* dst, nuint dstCapacity, void* src, nuint srcSize, uint frame)
    {
        ...
        {
            {
                var err_code = ZSTD_copyBlockSequences(&zc->seqCollector, ZSTD_getSeqStore(zc), zc->blockState.prevCBlock->rep);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
    ...
    private static void ZSTD_overflowCorrectIfNeeded(ZSTD_MatchState_t* ms, ZstdCwksp* ws, ZSTD_CCtx_params_s* @params, void* ip, void* iend)
    {
    ...
    private static nuint ZSTD_optimalBlockSize(ZSTD_CCtx_s* cctx, void* src, nuint srcSize, nuint blockSizeMax, int splitLevel, ZSTD_strategy strat, long savings)
    {
        ...
        if (splitLevel == 0)
        {
            assert(strat is >= ZSTD_strategy.ZSTD_fast and <= ZSTD_strategy.ZSTD_btultra2);
     ...
     *  @return : 0, or an error code
     */
    private static nuint ZSTD_loadDictionaryContent(ZSTD_MatchState_t* ms, ldmState_t* ls, ZstdCwksp* ws, ZSTD_CCtx_params_s* @params, void* src, nuint srcSize, ZSTD_dictTableLoadMethod_e dtlm, ZSTD_tableFillPurpose_e tfp)
    {
        var ip = (byte*)src;
        var iend = ip + srcSize;
        var loadLdmDict = @params->ldmParams.enableLdm == ZSTD_paramSwitch_e.ZSTD_ps_enable && ls != null ? 1 : 0;
     ...
     *                dictSize supposed >= 8
     */
    private static nuint ZSTD_loadZstdDictionary(ZSTD_compressedBlockState_t* bs, ZSTD_MatchState_t* ms, ZstdCwksp* ws, ZSTD_CCtx_params_s* @params, void* dict, nuint dictSize, ZSTD_dictTableLoadMethod_e dtlm, ZSTD_tableFillPurpose_e tfp, void* workspace)
    {
        var dictPtr = (byte*)dict;
        var dictEnd = dictPtr + dictSize;
        assert(dictSize >= 8);
        ...
        dictPtr += eSize;
        {
            var dictContentSize = (nuint)(dictEnd - dictPtr);
            {
                var err_code = ZSTD_loadDictionaryContent(ms, null, ws, @params, dictPtr, dictContentSize, dtlm, tfp);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
     ...
     *   @return : dictID, or an error code */
    private static nuint ZSTD_compress_insertDictionary(ZSTD_compressedBlockState_t* bs, ZSTD_MatchState_t* ms, ldmState_t* ls, ZstdCwksp* ws, ZSTD_CCtx_params_s* @params, void* dict, nuint dictSize, ZSTD_dictContentType_e dictContentType, ZSTD_dictTableLoadMethod_e dtlm, ZSTD_tableFillPurpose_e tfp, void* workspace)
    {
    ...
    private static nuint ZSTD_compressBegin_internal(ZSTD_CCtx_s* cctx, void* dict, nuint dictSize, ZSTD_dictContentType_e dictContentType, ZSTD_dictTableLoadMethod_e dtlm, ZSTD_CDict_s* cdict, ZSTD_CCtx_params_s* @params, ulong pledgedSrcSize, ZSTD_buffered_policy_e zbuff)
    {
        ...
        {
            var err_code = ZSTD_resetCCtx_internal(cctx, @params, pledgedSrcSize, dictContentSize, ZSTD_compResetPolicy_e.ZSTDcrp_makeClean, zbuff);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    private static nuint ZSTD_compressBegin_usingDict_deprecated(ZSTD_CCtx_s* cctx, void* dict, nuint dictSize, int compressionLevel)
    {
        ZSTD_CCtx_params_s cctxParams;
        {
            var @params = ZSTD_getParams_internal(compressionLevel, unchecked(0UL - 1), dictSize, ZSTD_CParamMode_e.ZSTD_cpm_noAttachDict);
    ...
    {
        {
            var err_code = ZSTD_compressBegin_internal(cctx, dict, dictSize, ZSTD_dictContentType_e.ZSTD_dct_auto, ZSTD_dictTableLoadMethod_e.ZSTD_dtlm_fast, null, @params, srcSize, ZSTD_buffered_policy_e.ZSTDb_not_buffered);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    {
        {
            var @params = ZSTD_getParams_internal(compressionLevel, srcSize, dict != null ? dictSize : 0, ZSTD_CParamMode_e.ZSTD_cpm_noAttachDict);
    ...
    {
        var cParams = ZSTD_getCParams_internal(compressionLevel, unchecked(0UL - 1), dictSize, ZSTD_CParamMode_e.ZSTD_cpm_createCDict);
    ...
    private static nuint ZSTD_initCDict_internal(ZSTD_CDict_s* cdict, void* dictBuffer, nuint dictSize, ZSTD_dictLoadMethod_e dictLoadMethod, ZSTD_dictContentType_e dictContentType, ZSTD_CCtx_params_s @params)
    {
        ...
        {
            var err_code = ZSTD_reset_matchState(&cdict->matchState, &cdict->workspace, &@params.cParams, @params.useRowMatchFinder, ZSTD_compResetPolicy_e.ZSTDcrp_makeClean, ZSTD_indexResetPolicy_e.ZSTDirp_reset, ZSTD_resetTarget_e.ZSTD_resetTarget_CDict);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
...

        {
            ...
            {
                var dictID = ZSTD_compress_insertDictionary(&cdict->cBlockState, &cdict->matchState, null, &cdict->workspace, &@params, cdict->dictContent, cdict->dictContentSize, dictContentType, ZSTD_dictTableLoadMethod_e.ZSTD_dtlm_full, ZSTD_tableFillPurpose_e.ZSTD_tfp_forCDict, cdict->entropyWorkspace);
                {
                    var err_code = dictID;
                    if (ERR_isError(err_code))
                    {
    ...
    private static ZSTD_CDict_s* ZSTD_createCDict_advanced_internal(nuint dictSize, ZSTD_dictLoadMethod_e dictLoadMethod, ZSTD_compressionParameters cParams, ZSTD_paramSwitch_e useRowMatchFinder, int enableDedicatedDictSearch, ZSTD_customMem customMem)
    {
        ...
        {
            var workspaceSize = ZSTD_cwksp_alloc_size((nuint)sizeof(ZSTD_CDict_s)) + ZSTD_cwksp_alloc_size((8 << 10) + 512) + ZSTD_sizeof_matchState(&cParams, useRowMatchFinder, enableDedicatedDictSearch, 0) + (dictLoadMethod == ZSTD_dictLoadMethod_e.ZSTD_dlm_byRef ? 0 : ZSTD_cwksp_alloc_size(ZSTD_cwksp_align(dictSize, (nuint)sizeof(void*))));
            ...
            var cdict = (ZSTD_CDict_s*)ZSTD_cwksp_reserve_object(&ws, (nuint)sizeof(ZSTD_CDict_s));
            assert(cdict != null);
    ...
    {
        var cctxParams = new ZSTD_CCtx_params_s();
    ...
    {
        var cParams = ZSTD_getCParams_internal(compressionLevel, unchecked(0UL - 1), dictSize, ZSTD_CParamMode_e.ZSTD_cpm_createCDict);
        var cdict = ZSTD_createCDict_advanced(dict, dictSize, ZSTD_dictLoadMethod_e.ZSTD_dlm_byCopy, ZSTD_dictContentType_e.ZSTD_dct_auto, cParams, ZSTD_defaultCMem);
        if (cdict != null)
        {
    ...
    {
        var cParams = ZSTD_getCParams_internal(compressionLevel, unchecked(0UL - 1), dictSize, ZSTD_CParamMode_e.ZSTD_cpm_createCDict);
        var cdict = ZSTD_createCDict_advanced(dict, dictSize, ZSTD_dictLoadMethod_e.ZSTD_dlm_byRef, ZSTD_dictContentType_e.ZSTD_dct_auto, cParams, ZSTD_defaultCMem);
        if (cdict != null)
        {
            cdict->compressionLevel = compressionLevel == 0 ? 3 : compressionLevel;
        }
    ...
    {
        var useRowMatchFinder = ZSTD_resolveRowMatchFinderMode(ZSTD_paramSwitch_e.ZSTD_ps_auto, &cParams);
        /* enableDedicatedDictSearch */
        var matchStateSize = ZSTD_sizeof_matchState(&cParams, useRowMatchFinder, 1, 0);
        var neededSize = ZSTD_cwksp_alloc_size((nuint)sizeof(ZSTD_CDict_s)) + (dictLoadMethod == ZSTD_dictLoadMethod_e.ZSTD_dlm_byRef ? 0 : ZSTD_cwksp_alloc_size(ZSTD_cwksp_align(dictSize, (nuint)sizeof(void*)))) + ZSTD_cwksp_alloc_size((8 << 10) + 512) + matchStateSize;
        ZSTD_CDict_s* cdict;
     ...
     */
    private static nuint ZSTD_compressBegin_usingCDict_internal(ZSTD_CCtx_s* cctx, ZSTD_CDict_s* cdict, ZstdFrameParameters fParams, ulong pledgedSrcSize)
    {
        ZSTD_CCtx_params_s cctxParams;
        if (cdict == null)
        {
     ...
     * cdict must be != NULL */
    public static nuint ZSTD_compressBegin_usingCDict_advanced(ZSTD_CCtx_s* cctx, ZSTD_CDict_s* cdict, ZstdFrameParameters fParams, ulong pledgedSrcSize)
    {
     ...
     */
    private static nuint ZSTD_compress_usingCDict_internal(ZSTD_CCtx_s* cctx, void* dst, nuint dstCapacity, void* src, nuint srcSize, ZSTD_CDict_s* cdict, ZstdFrameParameters fParams)
    {
        {
     ...
     * This function is DEPRECATED.
     */
    public static nuint ZSTD_compress_usingCDict_advanced(ZSTD_CCtx_s* cctx, void* dst, nuint dstCapacity, void* src, nuint srcSize, ZSTD_CDict_s* cdict, ZstdFrameParameters fParams)
    {
    ...
    public static nuint ZSTD_initCStream_usingCDict_advanced(ZSTD_CCtx_s* zcs, ZSTD_CDict_s* cdict, ZstdFrameParameters fParams, ulong pledgedSrcSize)
    {
        {
            var err_code = ZSTD_CCtx_reset(zcs, ZSTD_ResetDirective.ZSTD_reset_session_only);
            if (ERR_isError(err_code))
    ...
    public static nuint ZSTD_initCStream_usingDict(ZSTD_CCtx_s* zcs, void* dict, nuint dictSize, int compressionLevel)
    {
        ...
        {
            var err_code = ZSTD_CCtx_setParameter(zcs, ZstdCParameter.ZstdCCompressionLevel, compressionLevel);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    private static nuint ZSTD_compressStream_generic(ZSTD_CCtx_s* zcs, ZSTD_outBuffer_s* output, ZSTD_inBuffer_s* input, ZSTD_EndDirective flushMode)
    {
        ...
        while (someMoreWork != 0)
        {
            switch (zcs->streamStage)
            {
                ...
                {
                    var inputBuffered = zcs->appliedParams.inBufferMode == ZSTD_bufferMode_e.ZSTD_bm_buffered ? 1 : 0;
                    void* cDst;
                    nuint cSize;
                    ...
                    if (inputBuffered != 0)
                    {
                        var lastBlock = flushMode == ZSTD_EndDirective.ZSTD_e_end && ip == iend ? 1U : 0U;
                    ...
                    {
                        var lastBlock = flushMode == ZSTD_EndDirective.ZSTD_e_end && ip + iSize == iend ? 1U : 0U;
    ...
    {
        {
            var err_code = ZSTD_compressStream2(zcs, output, input, ZSTD_EndDirective.ZSTD_e_continue);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    private static nuint ZSTD_CCtx_init_compressStream2(ZSTD_CCtx_s* cctx, ZSTD_EndDirective endOp, nuint inSize)
    {
        ...
        if (@params.nbWorkers > 0)
        {
            ...
            {
                var err_code = ZSTDMT_initCStream_internal(cctx->mtctx, prefixDict.dict, prefixDict.dictSize, prefixDict.dictContentType, cctx->cdict, @params, cctx->pledgedSrcSizePlusOne - 1);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
        ...
        else
        {
            ...
            assert(!ERR_isError(ZSTD_checkCParams(@params.cParams)));
            {
                var err_code = ZSTD_compressBegin_internal(cctx, prefixDict.dict, prefixDict.dictSize, prefixDict.dictContentType, ZSTD_dictTableLoadMethod_e.ZSTD_dtlm_fast, cctx->cdict, &@params, pledgedSrcSize, ZSTD_buffered_policy_e.ZSTDb_buffered);
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
    ...
    public static nuint ZSTD_compressStream2(ZSTD_CCtx_s* cctx, ZSTD_outBuffer_s* output, ZSTD_inBuffer_s* input, ZSTD_EndDirective endOp)
    {
        ...
        if (cctx->appliedParams.nbWorkers > 0)
        {
            ...
            for (; ; )
            {
                ...
                {
                    assert(endOp is ZSTD_EndDirective.ZSTD_e_flush or ZSTD_EndDirective.ZSTD_e_end);
    ...
    public static nuint ZSTD_compress2(ZSTD_CCtx_s* cctx, void* dst, nuint dstCapacity, void* src, nuint srcSize)
    {
        ...
        cctx->requestedParams.outBufferMode = ZSTD_bufferMode_e.ZSTD_bm_stable;
        {
            nuint oPos = 0;
            nuint iPos = 0;
            var result = ZSTD_compressStream2_simpleArgs(cctx, dst, dstCapacity, &oPos, src, srcSize, &iPos, ZSTD_EndDirective.ZSTD_e_end);
    ...
    private static nuint ZSTD_transferSequences_wBlockDelim(ZSTD_CCtx_s* cctx, ZSTD_SequencePosition* seqPos, ZSTD_Sequence* inSeqs, nuint inSeqsSize, void* src, nuint blockSize, ZSTD_paramSwitch_e externalRepSearch)
    {
        ...
        for (; idx < inSeqsSize && (inSeqs[idx].matchLength != 0 || inSeqs[idx].offset != 0); ++idx)
        {
            ...
            if (cctx->appliedParams.validateSequences != 0)
            {
                ...
                {
                    var err_code = ZSTD_validateSequence(offBase, matchLength, cctx->appliedParams.cParams.minMatch, seqPos->posInSrc, cctx->appliedParams.cParams.windowLog, dictSize, ZSTD_hasExtSeqProd(&cctx->appliedParams));
                    if (ERR_isError(err_code))
                    {
                        return err_code;
                    }
    ...
    private static nuint ZSTD_compressSequences_internal(ZSTD_CCtx_s* cctx, void* dst, nuint dstCapacity, ZSTD_Sequence* inSeqs, nuint inSeqsSize, void* src, nuint srcSize)
    {
        ...
        while (remaining != 0)
        {
            ...
            var compressedSeqsSize = ZSTD_entropyCompressSeqStore(&cctx->seqStore, &cctx->blockState.prevCBlock->entropy, &cctx->blockState.nextCBlock->entropy, &cctx->appliedParams, op + ZSTD_blockHeaderSize, dstCapacity - ZSTD_blockHeaderSize, blockSize, cctx->tmpWorkspace, cctx->tmpWkspSize, cctx->bmi2);
            {
            ...
            else
            {
                ...
                var cBlockHeader = lastBlock + ((uint)blockType_e.bt_compressed << 1) + (uint)(compressedSeqsSize << 3);
                MEM_writeLE24(op, cBlockHeader);
    ...
    public static nuint ZSTD_compressSequences(ZSTD_CCtx_s* cctx, void* dst, nuint dstCapacity, ZSTD_Sequence* inSeqs, nuint inSeqsSize, void* src, nuint srcSize)
    {
        ...
        assert(cctx != null);
        {
            var err_code = ZSTD_CCtx_init_compressStream2(cctx, ZSTD_EndDirective.ZSTD_e_end, srcSize);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    private static nuint ZSTD_compressSequencesAndLiterals_internal(ZSTD_CCtx_s* cctx, void* dst, nuint dstCapacity, ZSTD_Sequence* inSeqs, nuint nbSequences, void* literals, nuint litSize, nuint srcSize)
    {
        ...
        nuint cSize = 0;
        var op = (byte*)dst;
        var repcodeResolution = cctx->appliedParams.searchForExternalRepcodes == ZSTD_paramSwitch_e.ZSTD_ps_enable ? 1 : 0;
        ...
        while (nbSequences != 0)
        {
            ...
            {
                var err_code = block.nbSequences;
                if (ERR_isError(err_code))
                {
                    return err_code;
                }
            ...
            var compressedSeqsSize = ZSTD_entropyCompressSeqStore_internal(op + ZSTD_blockHeaderSize, dstCapacity - ZSTD_blockHeaderSize, literals, block.litSize, &cctx->seqStore, &cctx->blockState.prevCBlock->entropy, &cctx->blockState.nextCBlock->entropy, &cctx->appliedParams, cctx->tmpWorkspace, cctx->tmpWkspSize, cctx->bmi2);
            {
                var err_code = compressedSeqsSize;
                if (ERR_isError(err_code))
                {
    ...
    public static nuint ZSTD_compressSequencesAndLiterals(ZSTD_CCtx_s* cctx, void* dst, nuint dstCapacity, ZSTD_Sequence* inSeqs, nuint inSeqsSize, void* literals, nuint litSize, nuint litCapacity, nuint decompressedSize)
    {
        ...
        {
            var err_code = ZSTD_CCtx_init_compressStream2(cctx, ZSTD_EndDirective.ZSTD_e_end, decompressedSize);
            if (ERR_isError(err_code))
            {
                return err_code;
            }
    ...
    {
        var nullInput = new ZSTD_inBuffer_s
        {
            src = null,
            size = 0,
            pos = 0
        };
        var stableInput = zcs->appliedParams.inBufferMode == ZSTD_bufferMode_e.ZSTD_bm_stable ? 1 : 0;
    ...
    {
        var input = inBuffer_forEndFlush(zcs);
        var remainingToFlush = ZSTD_compressStream2(zcs, output, &input, ZSTD_EndDirective.ZSTD_e_end);
        {
    ...
    {
        var cParams = ZSTD_getCParams_internal(compressionLevel, 0, dictSize, ZSTD_CParamMode_e.ZSTD_cpm_createCDict);
    ...
    private static ZSTD_parameters ZSTD_getParams_internal(int compressionLevel, ulong srcSizeHint, nuint dictSize, ZSTD_CParamMode_e mode)
    {
        ...
        var @params = new ZSTD_parameters
        {
            cParams = cParams
        };
        @params.fParams.contentSizeFlag = 1;

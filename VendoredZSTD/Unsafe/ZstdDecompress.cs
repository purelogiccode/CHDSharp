...
public static unsafe partial class Methods
{
    ...
    )
    {
        ...
        var newTable = (ZstdDDictS**)ZSTD_customCalloc(
            (nuint)sizeof(ZstdDDictS*) * newTableSize,
            customMem
        );
    ...
    {
        var ret = (ZstdDDictHashSet*)ZSTD_customMalloc(
            (nuint)sizeof(ZstdDDictHashSet),
            customMem
        );
        if (ret == null)
            return null;
        ret->ddictPtrTable = (ZstdDDictS**)ZSTD_customCalloc(
            (nuint)(64 * sizeof(ZstdDDictS*)),
            customMem
        );
    ...
    private static nuint ZSTD_startingInputLength(ZstdFormatE format)
    {
        ...
        assert(
            format == ZstdFormatE.ZstdFZstd1 || format == ZstdFormatE.ZstdFZstd1Magicless
        );
    ...
    )
    {
...

        {
            ...
            return minInputSize
                   ...
                   + ZstdDidFieldSize[dictId]
                   + ZstdFcsFieldSize[fcsId]
    ...
    )
    {
        ...
        memset(zfhPtr, 0, (uint)sizeof(ZstdFrameHeader));
        if (format != ZstdFormatE.ZstdFZstd1Magicless && MEM_readLE32(src) != 0xFD2FB528)
        {
            if ((MEM_readLE32(src) & 0xFFFFFFF0) == 0x184D2A50)
            {
                ...
                zfhPtr->frameType = ZstdFrameTypeE.ZstdSkippableFrame;
                return 0;
            }
...

        {
            ...
            {
                return unchecked(
                    (nuint)(-(int)ZstdErrorCode.ZstdErrorFrameParameterUnsupported)
                );
            }
            ...
            if (singleSegment == 0)
            {
                ...
                {
                    return unchecked(
                        (nuint)(-(int)ZstdErrorCode.ZstdErrorFrameParameterWindowTooLarge)
                    );
                }
    ...
    private static nuint ZSTD_decodeFrameHeader(ZstdDCtxS* dctx, void* src, nuint headerSize)
    {
        ...
        if (
            dctx->refMultipleDDicts == ZstdRefMultipleDDictsE.ZstdRmdRefMultipleDDicts
            && dctx->ddictSet != null
        )
        {
    ...
    private static ZstdFrameSizeInfo ZSTD_findFrameSizeInfo(void* src, nuint srcSize)
    {
        ZstdFrameSizeInfo frameSizeInfo;
        memset(&frameSizeInfo, 0, (uint)sizeof(ZstdFrameSizeInfo));
        ...
        ZstdFrameHeader zfh;
        {
            ...
            if (ret > 0)
            {
                return ZSTD_errorFrameSizeInfo(
                    unchecked((nuint)(-(int)ZstdErrorCode.ZstdErrorSrcSizeWrong))
                );
            }
        }
    ...
    )
    {
        ...
        if (
            remainingSrcSize
            < (nuint)(dctx->format == ZstdFormatE.ZstdFZstd1 ? 6 : 2) + ZstdBlockHeaderSize
        )
        {
            return unchecked((nuint)(-(int)ZstdErrorCode.ZstdErrorSrcSizeWrong));
        }

        {
            var frameHeaderSize = ZSTD_frameHeaderSize_internal(
                ip,
                (nuint)(dctx->format == ZstdFormatE.ZstdFZstd1 ? 5 : 1),
                dctx->format
            );
        ...
        if (dctx->fParams.checksumFlag != 0)
        {
            ...
            if (dctx->forceIgnoreChecksum == default)
            {
                ...
                uint checkRead = MEM_readLE32(ip);
    ...
    )
    {
        ...
        while (srcSize >= ZSTD_startingInputLength(dctx->format))
        {
            ...
            ZSTD_checkContinuity(dctx, dst, dstCapacity);
            {
                ...
                if (
                    ZSTD_getErrorCode(res) == ZstdErrorCode.ZstdErrorPrefixUnknown
                    && moreThan1Frame == 1
                )
                {
                    return unchecked((nuint)(-(int)ZstdErrorCode.ZstdErrorSrcSizeWrong));
    ...
    )
    {
        if (
            !(
                dctx->stage == ZstdDStage.ZstDdsDecompressBlock
                || dctx->stage == ZstdDStage.ZstDdsDecompressLastBlock
            )
        )
        {
            return dctx->expected;
    ...
    )
    {
        ...
        switch (dctx->stage)
        {
            ...
            {
                var errCode = ZSTD_decodeFrameHeader(
                    dctx,
                    dctx->headerBuffer,
                    dctx->headerSize
                );
            ...
            case ZstdDStage.ZstDdsDecodeBlockHeader:
            {
                ...
                if (cBlockSize != 0)
                {
                    dctx->stage =
                        bp.lastBlock != 0
                            ? ZstdDStage.ZstDdsDecompressLastBlock
                            : ZstdDStage.ZstDdsDecompressBlock;
                    return 0;
                }
...

            {
                if (dctx->validateChecksum != 0)
                {
                    ...
                    if (check32 != h32)
                        {
                            return unchecked(
                            (nuint)(-(int)ZstdErrorCode.ZstdErrorChecksumWrong)
                        );
                        }
                    }
    ...
    )
    {
        ...
        dictPtr += 8;
        {
            ...
            var workspaceSize = (nuint)(
                (sizeof(ZstdSeqSymbol) * 513)
                + (sizeof(ZstdSeqSymbol) * 257)
                + (sizeof(ZstdSeqSymbol) * 513)
            );
    ...
    {
        {
            var errCode = ZSTD_DCtx_loadDictionary_advanced(
                dctx,
                prefix,
                prefixSize,
                ZstdDictLoadMethodE.ZstdDlmByRef,
    ...
    public static nuint ZSTD_DCtx_refDDict(ZstdDCtxS* dctx, ZstdDDictS* ddict)
    {
        ...
        if (ddict != null)
        {
            ...
            if (dctx->refMultipleDDicts == ZstdRefMultipleDDictsE.ZstdRmdRefMultipleDDicts)
            {
                ...
                {
                    var errCode = ZSTD_DDictHashSet_addDDict(
                        dctx->ddictSet,
                        ddict,
                        dctx->customMem
                    );
    ...
    public static nuint ZSTD_DCtx_setParameter(ZstdDCtxS* dctx, ZstdDParameter dParam, int value)
    {
        ...
        switch (dParam)
        {
            ...
            {
                if (
                    ZSTD_dParam_withinBounds(ZstdDParameter.ZstdDExperimentalParam1, value)
                    == 0
                )
                    {
                        return unchecked(
            ...
            {
                if (
                    ZSTD_dParam_withinBounds(ZstdDParameter.ZstdDExperimentalParam2, value)
                    == 0
                )
                    {
                        return unchecked(
            ...
            {
                if (
                    ZSTD_dParam_withinBounds(ZstdDParameter.ZstdDExperimentalParam3, value)
                    == 0
                )
                    {
                        return unchecked(
            ...
            {
                if (
                    ZSTD_dParam_withinBounds(ZstdDParameter.ZstdDExperimentalParam4, value)
                    == 0
                )
                    {
                        return unchecked(
            ...
            {
                if (
                    ZSTD_dParam_withinBounds(ZstdDParameter.ZstdDExperimentalParam5, value)
                    == 0
                )
                    {
                        return unchecked(
    ...
    )
    {
        ...
        while (someMoreWork != 0)
        {
            switch (zds->streamStage)
            {
                ...
                case ZstdDStreamStage.ZdssLoadHeader:
                {
                    ...
                    if (hSize != 0)
                    {
                        ...
                        if (toLoad > remainingInput)
                        {
                            ...
                            {
                                var errCode = ZSTD_getFrameHeader_advanced(
                                    &zds->fParams,
                                    zds->headerBuffer,
                                    zds->lhSize,
                                    zds->format
                            ...
                            return (
                                       (nuint)(zds->format == ZstdFormatE.ZstdFZstd1 ? 6 : 2)
                                       > hSize
                                           ? (nuint)(
                                               zds->format == ZstdFormatE.ZstdFZstd1 ? 6 : 2
                                           )
                                           : hSize
                                   )
                                   - zds->lhSize
                    ...
                    if (
                        ...
                        && zds->fParams.frameType != ZstdFrameTypeE.ZstdSkippableFrame
                        && (nuint)(oend - op) >= zds->fParams.frameContentSize
                    )
                    {
                        var cSize = ZSTD_findFrameCompressedSize(istart, (nuint)(iend - istart));
                    ...
                    if (
                        zds->outBufferMode == ZstdBufferModeE.ZstdBmStable
                        && zds->fParams.frameType != ZstdFrameTypeE.ZstdSkippableFrame
                        && zds->fParams.frameContentSize != unchecked(0UL - 1)
                        && (nuint)(oend - op) < zds->fParams.frameContentSize
                    )
                    ...
                    var neededOutBuffSize =
                        zds->outBufferMode == ZstdBufferModeE.ZstdBmBuffered
                                ...
                                if (bufferSize > zds->staticSize - (nuint)sizeof(ZstdDCtxS))
                                    {
                                        return unchecked(
                                        (nuint)(
                                            -(int)ZstdErrorCode.ZstdErrorMemoryAllocation
                                        )
                                    );
                                    }
                                }
                ...
                case ZstdDStreamStage.ZdssRead:
                {
                    ...
                    {
                        {
                            var errCode = ZSTD_decompressContinueStream(
                                zds,
                                &op,
                                oend,
                                ip,
        ...
        if (ip == istart && op == ostart)
        {
            ...
            if (zds->noForwardProgress >= 16)
            {
                ...
                {
                    return unchecked(
                        (nuint)(-(int)ZstdErrorCode.ZstdErrorNoForwardProgressDestFull)
                    );
                }
                ...
                {
                    return unchecked(
                        (nuint)(-(int)ZstdErrorCode.ZstdErrorNoForwardProgressInputEmpty)
                    );
                }
...

        {
            ...
            nextSrcSizeHint +=
                ZstdBlockHeaderSize
                * (nuint)(ZSTD_nextInputType(zds) == ZstdNextInputTypeE.ZstDnitBlock ? 1 : 0);

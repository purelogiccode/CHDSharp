...
public static unsafe partial class Methods
{
    ...
    private static void ZDICT_countEStats(EStatsRessT esr, ZstdParameters* @params, uint* countLit, uint* offsetcodeCount, uint* matchlengthCount, uint* litlengthCount, uint* repOffsets, void* src, nuint srcSize, uint notificationLevel)
    {
        var blockSizeMax = (nuint)(1 << 17 < 1 << (int)@params->cParams.windowLog ? 1 << 17 : 1 << (int)@params->cParams.windowLog);
        ...
        var cSize = ZSTD_compressBlock_deprecated(esr.zc, esr.workPlace, 1 << 17, src, srcSize);
        ...
        if (cSize != 0)
        {
            var seqStorePtr = ZSTD_getSeqStore(esr.zc);
            {
                byte* bytePtr;
...

            {
                ...
                {
                    var codePtr = seqStorePtr->ofCode;
                    uint u;
                    for (u = 0; u < nbSeq; u++)
                    {
                ...
                {
                    var codePtr = seqStorePtr->mlCode;
                    uint u;
                    for (u = 0; u < nbSeq; u++)
                    {
                ...
                {
                    var codePtr = seqStorePtr->llCode;
                    uint u;
                    for (u = 0; u < nbSeq; u++)
                    {
                ...
                if (nbSeq >= 2)
                {
                    var seq = seqStorePtr->sequencesStart;
                    var offset1 = seq[0].offBase - 3;
                    var offset2 = seq[1].offBase - 3;
                    if (offset1 >= 1024)
                    {
                        offset1 = 0;
                    }
    ...
    {
        var countLit = stackalloc uint[256];
        /* no final ; */
        var hufTable = stackalloc nuint[257];
        var offcodeCount = stackalloc uint[31];
        var offcodeNCount = stackalloc short[31];
        var offcodeMax = ZSTD_highbit32((uint)(dictBufferSize + 128 * (1 << 10)));
        var matchLengthCount = stackalloc uint[53];
        ...
        uint u, huffLog = 11, offlog = 8, mlLog = 9, llLog = 9;
        nuint pos = 0;
        nuint eSize = 0;
        var totalSrcSize = ZDICT_totalSampleSize(fileSizes, nbFiles);
        var averageSampleSize = totalSrcSize / (nbFiles + (uint)(nbFiles == 0 ? 1 : 0));
        var dstPtr = (byte*)dstBuffer;
        ...
        var @params = ZSTD_getParams(compressionLevel, averageSampleSize, dictBufferSize);
        esr.dict = ZSTD_createCDict_advanced(dictBuffer, dictBufferSize, ZstdDictLoadMethodE.ZstdDlmByRef, ZstdDictContentTypeE.ZstdDctRawContent, @params.cParams, ZstdDefaultCMem);
        ...
        {
            var maxNbBits = HUF_buildCTable_wksp(hufTable, countLit, 255, huffLog, wksp, sizeof(uint) * 1216);
        ...
        var errorCode = FSE_normalizeCount(offcodeNCount, offlog, offcodeCount, total, offcodeMax, 1);
        if (ERR_isError(errorCode))
        {
            eSize = errorCode;
            goto _cleanup;
        ...
        {
            var hhSize = HUF_writeCTable_wksp(dstPtr, maxDstSize, hufTable, 255, huffLog, wksp, sizeof(uint) * 1216);
        ...
        {
            var mhSize = FSE_writeNCount(dstPtr, maxDstSize, matchLengthNCount, 52, mlLog);
        ...
        {
            var lhSize = FSE_writeNCount(dstPtr, maxDstSize, litLengthNCount, 35, llLog);
     ...
     */
    public static nuint ZDICT_finalizeDictionary(void* dictBuffer, nuint dictBufferCapacity, void* customDictContent, nuint dictContentSize, void* samplesBuffer, nuint* samplesSizes, uint nbSamples, ZdictParamsT @params)
    {
        var header = stackalloc byte[256];
        ...
        nuint hSize = 8;
        {
            var eSize = ZDICT_analyzeEntropy(header + hSize, 256 - hSize, compressionLevel, samplesBuffer, samplesSizes, nbSamples, customDictContent, dictContentSize, notificationLevel);
        ...
        {
            var dictSize = hSize + paddingSize + dictContentSize;
            /* The dictionary consists of the header, optional padding, and the content.
             * The padding comes before the content because the "best" position in the
             * dictionary is the last byte.
             */
            var outDictHeader = (byte*)dictBuffer;
            var outDictPadding = outDictHeader + hSize;
            var outDictContent = outDictPadding + paddingSize;
    ...
    private static nuint ZDICT_addEntropyTablesFromBuffer_advanced(void* dictBuffer, nuint dictContentSize, nuint dictBufferCapacity, void* samplesBuffer, nuint* samplesSizes, uint nbSamples, ZdictParamsT @params)
    {
        var compressionLevel = @params.compressionLevel == 0 ? 3 : @params.compressionLevel;
        var notificationLevel = @params.notificationLevel;
        nuint hSize = 8;
        {
            var eSize = ZDICT_analyzeEntropy((sbyte*)dictBuffer + hSize, dictBufferCapacity - hSize, compressionLevel, samplesBuffer, samplesSizes, nbSamples, (sbyte*)dictBuffer + dictBufferCapacity - dictContentSize, dictContentSize, notificationLevel);
            if (ZDICT_isError(eSize))
                return eSize;
        ...
        {
            var randomId = ZSTD_XXH64((sbyte*)dictBuffer + dictBufferCapacity - dictContentSize, dictContentSize, 0);
            var compliantId = (uint)(randomId % ((1U << 31) - 32768) + 32768);
            var dictId = @params.dictID != 0 ? @params.dictID : compliantId;
    ...
    {
        var @params = new ZdictFastCoverParamsT
        {
            d = 8,
            steps = 4
        };

...
{
    private static readonly rawSeqStore_t kNullRawSeqStore = new(
        null,
        0,
        0,
        0,
    ...
    )
    {
        ...
        if (blockEndIdx > maxDist + loadedDictEnd)
        {
            ...
            if (window->dictLimit < window->lowLimit) window->dictLimit = window->lowLimit;
    ...
    )
    {
        ...
        {
            var highInputIdx = (nint)(ip + srcSize - window->dictBase);
            var lowLimitMax =
                highInputIdx > (nint)window->dictLimit ? window->dictLimit : (uint)highInputIdx;

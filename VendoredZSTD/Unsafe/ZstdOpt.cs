...
public static unsafe partial class Methods
{
    ...
    private static uint ZSTD_downscaleStats(uint* table, uint lastEltIndex, uint shift, BaseDirectiveE base1)
    {
        uint s, sum = 0;
        assert(shift < 30);
        for (s = 0; s < lastEltIndex + 1; s++)
        {
            var @base = (uint)(base1 != default ? 1 : table[s] > 0 ? 1 : 0);
    ...
    private static uint ZSTD_scaleStats(uint* table, uint lastEltIndex, uint logTarget)
    {
        ...
        return ZSTD_downscaleStats(table, lastEltIndex, ZSTD_highbit32(factor), BaseDirectiveE.Base1Guaranteed);
    }
     ...
     */
    private static void ZSTD_rescaleFreqs(OptStateT* optPtr, byte* src, nuint srcSize, int optLevel)
    {
        ...
        if (optPtr->litLengthSum == 0)
        {
            ...
            if (optPtr->symbolCosts->huf.repeatMode == HufRepeat.HufRepeatValid)
            {
                ...
                if (compressedLiterals != 0)
                {
                    ...
                    for (lit = 0; lit <= (1 << 8) - 1; lit++)
                    {
                        ...
                        var bitCost = HUF_getNbBitsFromCTable(&optPtr->symbolCosts->huf.CTable.e0, lit);
            ...
            else
            {
                ...
                if (compressedLiterals != 0)
                {
                    ...
                    optPtr->litSum = ZSTD_downscaleStats(optPtr->litFreq, (1 << 8) - 1, 8, BaseDirectiveE.Base0Possible);
                }
    ...
    private static uint ZSTD_rawLiteralsCost(byte* literals, uint litLength, OptStateT* optPtr, int optLevel)
    {
        if (litLength == 0)
            return 0;
        if (ZSTD_compressedLiterals(optPtr) == 0)
...

        {
            ...
            {
                var litPrice = optLevel != 0 ? ZSTD_fracWeight(optPtr->litFreq[literals[u]]) : ZSTD_bitWeight(optPtr->litFreq[literals[u]]);
    ...
    private static uint ZSTD_litLengthPrice(uint litLength, OptStateT* optPtr, int optLevel)
    {
...

        {
            ...
            return (uint)(LlBits[llCode] * (1 << 8)) + optPtr->litLengthSumBasePrice - (optLevel != 0 ? ZSTD_fracWeight(optPtr->litLengthFreq[llCode]) : ZSTD_bitWeight(optPtr->litLengthFreq[llCode]));
        }
    }
     ...
     */
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static uint ZSTD_getMatchPrice(uint offBase, uint matchLength, OptStateT* optPtr, int optLevel)
    {
        ...
        var price = offCode * (1 << 8) + (optPtr->offCodeSumBasePrice - (optLevel != 0 ? ZSTD_fracWeight(optPtr->offCodeFreq[offCode]) : ZSTD_bitWeight(optPtr->offCodeFreq[offCode])));
...

        {
            ...
            price += (uint)(MlBits[mlCode] * (1 << 8)) + (optPtr->matchLengthSumBasePrice - (optLevel != 0 ? ZSTD_fracWeight(optPtr->matchLengthFreq[mlCode]) : ZSTD_bitWeight(optPtr->matchLengthFreq[mlCode])));
        }
    ...
    private static void ZSTD_updateStats(OptStateT* optPtr, uint litLength, byte* literals, uint offBase, uint matchLength)
    {
        if (ZSTD_compressedLiterals(optPtr) != 0)
        {
            uint u;
    ...
    private static uint ZSTD_insertAndFindFirstIndexHash3(ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip)
    {
    ...
    private static uint ZSTD_insertBt1(ZstdMatchStateT* ms, byte* ip, byte* iend, uint target, uint mls, int extDict)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void ZSTD_updateTree_internal(ZstdMatchStateT* ms, byte* ip, byte* iend, uint mls, ZstdDictModeE dictMode)
    {
        var @base = ms->window.@base;
        var target = (uint)(ip - @base);
        ...
        while (idx < target)
        {
            var forward = ZSTD_insertBt1(ms, @base + idx, iend, target, mls, dictMode == ZstdDictModeE.ZstdExtDict ? 1 : 0);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static uint ZSTD_insertBtAndGetAllMatches(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iLimit, ZstdDictModeE dictMode, uint* rep, uint ll0, uint lengthToBeat, uint mls)
    {
        var cParams = &ms->cParams;
        var sufficientLen = cParams->targetLength < (1 << 12) - 1 ? cParams->targetLength : (1 << 12) - 1;
        var @base = ms->window.@base;
        var curr = (uint)(ip - @base);
        var hashLog = cParams->hashLog;
        ...
        var dmsHighLimit = dictMode == ZstdDictModeE.ZstdDictMatchState ? (uint)(dmsEnd - dmsBase) : 0;
        var dmsLowLimit = dictMode == ZstdDictModeE.ZstdDictMatchState ? dms->window.lowLimit : 0;
        var dmsIndexDelta = dictMode == ZstdDictModeE.ZstdDictMatchState ? windowLow - dmsHighLimit : 0;
        var dmsHashLog = dictMode == ZstdDictModeE.ZstdDictMatchState ? dmsCParams->hashLog : hashLog;
        var dmsBtLog = dictMode == ZstdDictModeE.ZstdDictMatchState ? dmsCParams->chainLog - 1 : btLog;
        var dmsBtMask = dictMode == ZstdDictModeE.ZstdDictMatchState ? (1U << (int)dmsBtLog) - 1 : 0;
        var dmsBtLow = dictMode == ZstdDictModeE.ZstdDictMatchState && dmsBtMask < dmsHighLimit - dmsLowLimit ? dmsHighLimit - dmsBtMask : dmsLowLimit;
        ...
        if (mls == 3 && bestLength < mls)
        {
            ...
            {
                nuint mlen;
                if (dictMode == ZstdDictModeE.ZstdNoDict || dictMode == ZstdDictModeE.ZstdDictMatchState || matchIndex3 >= dictLimit)
                {
        ...
        for (; nbCompares != 0 && matchIndex >= matchLow; --nbCompares)
        {
            ...
            if (dictMode == ZstdDictModeE.ZstdNoDict || dictMode == ZstdDictModeE.ZstdDictMatchState || matchIndex + matchLength >= dictLimit)
            {
                assert(matchIndex + matchLength >= dictLimit);
                match = @base + matchIndex;
#if DEBUG
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static uint ZSTD_btGetAllMatches_internal(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat, ZstdDictModeE dictMode, uint mls)
    {
    ...
    private static uint ZSTD_btGetAllMatches_noDict_3(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdNoDict, 3);
    }

    private static uint ZSTD_btGetAllMatches_noDict_4(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdNoDict, 4);
    }

    private static uint ZSTD_btGetAllMatches_noDict_5(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdNoDict, 5);
    }

    private static uint ZSTD_btGetAllMatches_noDict_6(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdNoDict, 6);
    }

    private static uint ZSTD_btGetAllMatches_extDict_3(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdExtDict, 3);
    }

    private static uint ZSTD_btGetAllMatches_extDict_4(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdExtDict, 4);
    }

    private static uint ZSTD_btGetAllMatches_extDict_5(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdExtDict, 5);
    }

    private static uint ZSTD_btGetAllMatches_extDict_6(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdExtDict, 6);
    }

    private static uint ZSTD_btGetAllMatches_dictMatchState_3(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdDictMatchState, 3);
    }

    private static uint ZSTD_btGetAllMatches_dictMatchState_4(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdDictMatchState, 4);
    }

    private static uint ZSTD_btGetAllMatches_dictMatchState_5(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
        return ZSTD_btGetAllMatches_internal(matches, ms, nextToUpdate3, ip, iHighLimit, rep, ll0, lengthToBeat, ZstdDictModeE.ZstdDictMatchState, 5);
    }

    private static uint ZSTD_btGetAllMatches_dictMatchState_6(ZstdMatchT* matches, ZstdMatchStateT* ms, uint* nextToUpdate3, byte* ip, byte* iHighLimit, uint* rep, uint ll0, uint lengthToBeat)
    {
     ...
     */
    private static void ZSTD_opt_getNextMatchAndUpdateSeqStore(ZstdOptLdmT* optLdm, uint currPosInBlock, uint blockBytesRemaining)
    {
        ...
        var currSeq = optLdm->seqStore.seq[optLdm->seqStore.pos];
        assert(optLdm->seqStore.posInSequence <= currSeq.litLength + currSeq.matchLength);
        var currBlockEndPos = currPosInBlock + blockBytesRemaining;
        var literalsBytesRemaining = optLdm->seqStore.posInSequence < currSeq.litLength ? currSeq.litLength - (uint)optLdm->seqStore.posInSequence : 0;
        var matchBytesRemaining = literalsBytesRemaining == 0 ? currSeq.matchLength - ((uint)optLdm->seqStore.posInSequence - currSeq.litLength) : currSeq.matchLength;
     ...
     */
    private static void ZSTD_optLdm_maybeAddMatch(ZstdMatchT* matches, uint* nbMatches, ZstdOptLdmT* optLdm, uint currPosInBlock, uint minMatch)
    {
     ...
     */
    private static void ZSTD_optLdm_processMatchCandidate(ZstdOptLdmT* optLdm, ZstdMatchT* matches, uint* nbMatches, uint currPosInBlock, uint remainingBytes, uint minMatch)
    {
        if (optLdm->seqStore.size == 0 || optLdm->seqStore.pos >= optLdm->seqStore.size)
        {
            return;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static nuint ZSTD_compressBlock_opt_generic(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize, int optLevel, ZstdDictModeE dictMode)
    {
        ...
        ZstdOptLdmT optLdm;
        var lastStretch = new ZstdOptimalT();
        ...
        while (ip < ilimit)
        {
            uint cur, lastPos;
            {
                ...
                var nbMatches = getAllMatches(matches, ms, &nextToUpdate3, ip, iend, rep, ll0, minMatch);
                ZSTD_optLdm_processMatchCandidate(&optLdm, matches, &nbMatches, (uint)(ip - istart), (uint)(iend - ip), minMatch);
                if (nbMatches == 0)
                {
                    ip++;
                    continue;
                ...
                assert(opt[0].price >= 0);
                {
                    ...
                    for (matchNb = 0; matchNb < nbMatches; matchNb++)
                    {
                        ...
                        for (; pos <= end; pos++)
                        {
                            var matchPrice = (int)ZSTD_getMatchPrice(offBase, pos, optStatePtr, optLevel);
            ...
            for (cur = 1; cur <= lastPos; cur++)
            {
                ...
                assert(cur <= 1 << 12);
                {
                    ...
                    var price = opt[cur - 1].price + (int)ZSTD_rawLiteralsCost(ip + cur - 1, 1, optStatePtr, optLevel) + ((int)ZSTD_litLengthPrice(litlen, optStatePtr, optLevel) - (int)ZSTD_litLengthPrice(litlen - 1, optStatePtr, optLevel));
                    ...
                    if (price <= opt[cur].price)
                    {
                        ...
                        if (optLevel >= 1 && prevMatch.litlen == 0 && (int)ZSTD_litLengthPrice(1, optStatePtr, optLevel) - (int)ZSTD_litLengthPrice(1 - 1, optStatePtr, optLevel) < 0 && ip + cur < iend)
                        {
                            ...
                            var with1Literal = prevMatch.price + (int)ZSTD_rawLiteralsCost(ip + cur, 1, optStatePtr, optLevel) + ((int)ZSTD_litLengthPrice(1, optStatePtr, optLevel) - (int)ZSTD_litLengthPrice(1 - 1, optStatePtr, optLevel));
                            var withMoreLiterals = price + (int)ZSTD_rawLiteralsCost(ip + cur, 1, optStatePtr, optLevel) + ((int)ZSTD_litLengthPrice(litlen + 1, optStatePtr, optLevel) - (int)ZSTD_litLengthPrice(litlen + 1 - 1, optStatePtr, optLevel));
                            if (with1Literal < withMoreLiterals && with1Literal < opt[cur + 1].price)
                            {
                                ...
                                opt[cur + 1] = prevMatch;
                                memcpy(opt[cur + 1].rep, &newReps, (uint)sizeof(RepcodesS));
                                opt[cur + 1].litlen = 1;
                ...
                assert(opt[cur].price >= 0);
                {
                    ...
                    var basePrice = previousPrice + (int)ZSTD_litLengthPrice(0, optStatePtr, optLevel);
                    var nbMatches = getAllMatches(matches, ms, &nextToUpdate3, inr, iend, opt[cur].rep, ll0, minMatch);
                    uint matchNb;
                    ZSTD_optLdm_processMatchCandidate(&optLdm, matches, &nbMatches, (uint)(inr - istart), (uint)(iend - inr), minMatch);
                    if (nbMatches == 0)
                    {
                        continue;
                    }

                    {
                        ...
                        if (longestMl > sufficientLen || cur + longestMl >= 1 << 12 || ip + cur + longestMl >= iend)
                        {
                            lastStretch.mlen = longestMl;
                            lastStretch.off = matches[nbMatches - 1].off;
                            lastStretch.litlen = 0;
                            lastPos = cur + longestMl;
                            goto _shortestPath;
                        }
                    }
                    ...
                    for (matchNb = 0; matchNb < nbMatches; matchNb++)
                    {
                        ...
                        {
                            var pos = cur + mlen;
                            var price = basePrice + (int)ZSTD_getMatchPrice(offset, mlen, optStatePtr, optLevel);
            ...
            lastStretch = opt[lastPos];
            assert(cur >= lastStretch.mlen);
            cur = lastPos - lastStretch.mlen;
            _shortestPath:
            assert(opt[0].mlen == 0);
            assert(lastPos >= lastStretch.mlen);
            assert(cur == lastPos - lastStretch.mlen);
            if (lastStretch.mlen == 0)
            {
                assert(lastStretch.litlen == (uint)(ip - anchor) + lastPos);
                ip += lastPos;
                continue;
            }
            ...
            {
                memcpy(rep, lastStretch.rep, (uint)sizeof(RepcodesS));
    ...
    private static nuint ZSTD_compressBlock_opt0(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize, ZstdDictModeE dictMode)
    {
        return ZSTD_compressBlock_opt_generic(ms, seqStore, rep, src, srcSize, 0, dictMode);
    }

    private static nuint ZSTD_compressBlock_opt2(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize, ZstdDictModeE dictMode)
    {
        return ZSTD_compressBlock_opt_generic(ms, seqStore, rep, src, srcSize, 2, dictMode);
    }

    private static nuint ZSTD_compressBlock_btopt(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
        return ZSTD_compressBlock_opt0(ms, seqStore, rep, src, srcSize, ZstdDictModeE.ZstdNoDict);
    }
     ...
     */
    private static void ZSTD_initStats_ultra(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
    ...
    private static nuint ZSTD_compressBlock_btultra(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
        return ZSTD_compressBlock_opt2(ms, seqStore, rep, src, srcSize, ZstdDictModeE.ZstdNoDict);
    }

    /* note : no btultra2 variant for extDict nor dictMatchState,
     * because btultra2 is not meant to work with dictionaries
     * and is only specific for the first block (no prefix) */
    private static nuint ZSTD_compressBlock_btultra2(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
    ...
    private static nuint ZSTD_compressBlock_btopt_dictMatchState(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
        return ZSTD_compressBlock_opt0(ms, seqStore, rep, src, srcSize, ZstdDictModeE.ZstdDictMatchState);
    }

    private static nuint ZSTD_compressBlock_btopt_extDict(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
        return ZSTD_compressBlock_opt0(ms, seqStore, rep, src, srcSize, ZstdDictModeE.ZstdExtDict);
    }

    private static nuint ZSTD_compressBlock_btultra_dictMatchState(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
        return ZSTD_compressBlock_opt2(ms, seqStore, rep, src, srcSize, ZstdDictModeE.ZstdDictMatchState);
    }

    private static nuint ZSTD_compressBlock_btultra_extDict(ZstdMatchStateT* ms, SeqStoreT* seqStore, uint* rep, void* src, nuint srcSize)
    {
        return ZSTD_compressBlock_opt2(ms, seqStore, rep, src, srcSize, ZstdDictModeE.ZstdExtDict);
    }
}
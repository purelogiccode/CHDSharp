...
{
    private static readonly buffer_s g_nullBuffer = new(start: null, capacity: 0);
    ...
    {
        var bufPool = (ZSTDMT_bufferPool_s*)ZSTD_customCalloc((nuint)sizeof(ZSTDMT_bufferPool_s), cMem);
        if (bufPool == null)
            return null;
    ...
    {
        var poolSize = (nuint)sizeof(ZSTDMT_bufferPool_s);
    ...
    {
        var cctxPool = (ZSTDMT_CCtxPool*)ZSTD_customCalloc((nuint)sizeof(ZSTDMT_CCtxPool), cMem);
        assert(nbWorkers > 0);
        if (cctxPool == null)
            return null;
    ...
    private static nuint ZSTDMT_sizeof_CCtxPool(ZSTDMT_CCtxPool* cctxPool)
    {
        SynchronizationWrapper.Enter(&cctxPool->poolMutex);
        {
            ...
            var poolSize = (nuint)sizeof(ZSTDMT_CCtxPool);
            var arraySize = (nuint)(cctxPool->totalCCtx * sizeof(ZSTD_CCtx_s*));
    ...
    private static void ZSTDMT_compressionJob(void* jobDescription)
    {
        ...
        if (job->cdict != null)
        {
            var initError = ZSTD_compressBegin_advanced_internal(cctx, null, 0, ZSTD_dictContentType_e.ZSTD_dct_auto, ZSTD_dictTableLoadMethod_e.ZSTD_dtlm_fast, job->cdict, &jobParams, job->fullFrameSize);
        ...
        else
        {
            ...
            {
                var initError = ZSTD_compressBegin_advanced_internal(cctx, job->prefix.start, job->prefix.size, ZSTD_dictContentType_e.ZSTD_dct_rawContent, ZSTD_dictTableLoadMethod_e.ZSTD_dtlm_fast, null, &jobParams, pledgedSrcSize);
                if (ERR_isError(initError))
                {
                    SynchronizationWrapper.Enter(&job->job_mutex);
                    job->cSize = initError;
    ...
    private static readonly RoundBuff_t kNullRoundBuff = new(buffer: null, capacity: 0, pos: 0);
    ...
    private static ZSTDMT_jobDescription* ZSTDMT_createJobsTable(uint* nbJobsPtr, ZSTD_customMem cMem)
    {
        ...
        uint jobNb;
        var jobTable = (ZSTDMT_jobDescription*)ZSTD_customCalloc(nbJobs * (uint)sizeof(ZSTDMT_jobDescription), cMem);
        var initError = 0;
        if (jobTable == null)
            return null;
    ...
    private static ZSTDMT_CCtx_s* ZSTDMT_createCCtx_advanced_internal(uint nbWorkers, ZSTD_customMem cMem, void* pool)
    {
        ...
        var mtctx = (ZSTDMT_CCtx_s*)ZSTD_customCalloc((nuint)sizeof(ZSTDMT_CCtx_s), cMem);
        if (mtctx == null)
            return null;
    ...
    private static void ZSTDMT_updateCParams_whileCompressing(ZSTDMT_CCtx_s* mtctx, ZSTD_CCtx_params_s* cctxParams)
    {
        ...
        var saved_wlog = mtctx->@params.cParams.windowLog;

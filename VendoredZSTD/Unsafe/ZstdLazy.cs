...
public static unsafe partial class Methods
{
    ...
    )
    {
        ...
        const uint cacheSize = bucketSize - 1;
    ...
    )
    {
        ...
        var dms = ms->dictMatchState;
        var ddsHashLog =
            dictMode == ZstdDictModeE.ZstdDedicatedDictSearch ? dms->cParams.hashLog - 2 : 0;
        var ddsIdx =
            dictMode == ZstdDictModeE.ZstdDedicatedDictSearch
    ...
    {
        var comparisonMask = Vector128.Create((sbyte)tag);
        var matches = stackalloc int[4];
...
#pragma warning disable CA1857, CA1857
                var t3 = AdvSimd.ShiftRightLogicalAdd(t2, t2, 28).As<ulong, byte>();
#pragma warning restore CA1857, CA1857
    ...
    )
    {
        ...
        else if (dictMode == ZstdDictModeE.ZstdDictMatchState)
        {
            ...
            var dmsIndexDelta = dictLimit - dmsSize;
            {
                ...
                nuint numMatches = 0;
                nuint currMatch = 0;
                var matches = ZSTD_row_getMatchMask(
                    dmsTagRow,
                    (byte)dmsTag,
                    headGrouped,
                    rowEntries
    ...
    )
    {
        ...
        while (ip < ilimit)
        {
            ...
            if (
                ...
                && offset1 > 0
                && MEM_read32(ip + 1 - offset1) == MEM_read32(ip + 1)
            )
            {
            ...
            if (depth >= 1)
                while (ip < ilimit)
                {
                    ...
                    if (
                        ...
                        && offBase != 0
                        && offset1 > 0
                        && MEM_read32(ip) == MEM_read32(ip - offset1)
                    )
                    {
    ...
    )
    {
        return ZSTD_compressBlock_lazy_generic(
            ...
            src,
            srcSize,
            SearchMethodE.SearchBinaryTree,
            2,

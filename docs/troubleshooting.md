...
A: The CLI (`CHDSharpCli`) and the WPF tester (`CHDSharpTester`) report automatically: every Serilog event at `Warning` or above is POSTed fire-and-forget to the Bug Report API with environment details (date, app name/version, OS/architecture/bitness, Windows version, processor count, base directory, temp path), the error message, and exception details (type, message, source, stack trace). User cancellation is excluded and throttling/failures are tolerated, so reporting never interrupts the app. For a manual report, include the `ChdError`, the CHD version, the codec list (from header info), the hunk number from the log output if present, and attach the matching rolling log file (`%LocalAppData%\CHDSharp\logs\` or `%LocalAppData%\CHDSharpTester\logs\`). Enabling `Chd.LoggerFactory` with a file sink before reproducing usually captures the root cause.

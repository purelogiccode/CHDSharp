...
public partial class ZLib : IZLib
{
    ...
    /// <param name="level">The compression level. It must be <see cref="ZDefaultCompression" />, or between 0 and 9: 1 gives best speed, 9 gives best compression, 0 gives no compression at all (the input data is simply copied a block at a time). <see cref="ZDefaultCompression" /> requests a default compromise between speed and compression (equivalent to level 6).</param>
    /// <returns><see cref="ZOk"/> if success, <see cref="ZMemError"/> if there was not enough memory, or <see cref="ZStreamError"/> if level is not a valid compression level.</returns>
    ...
    /// <param name="strategy">Used to tune the compression algorithm. Use the value <see cref="ZDefaultStrategy"/> for normal data, <see cref="ZFiltered"/> for data produced by a filter(or predictor), <see cref="ZHuffmanOnly"/> to force Huffman encoding only (no string match), or <see cref="ZRle"/> to limit match distances to one (run-length encoding).</param>
    /// <returns><see cref="ZOk"/> if success, <see cref="ZMemError"/> if there was not enough memory, or <see cref="ZStreamError"/> if any parameter is invalid (such as an invalid method).</returns>
    /// <remarks>This method does not perform any compression. Actual compression will be done by <see cref="Deflate(ref ZStream, int)"/>.</remarks>
    ...
    /// <para>Before calling this method, the application should ensure that at least one of the actions is possible, by providing more input and/or consuming more output, and updating Next* and Available* properties accordingly; <see cref="ZStream.AvailableOut"/> should never be zero before the call.  The application can consume the compressed output when it wants, for example when the output buffer is full (<see cref="ZStream.AvailableOut"/> == 0), or after each call of the method. If the method returns <see cref="ZOk" /> and the <see cref="ZStream.AvailableOut"/> property of <paramref name="strm" /> is zero, it must be called again after making room in the output buffer because there might be more output pending.</para>
    /// </summary>
    /// <param name="strm">The stream to be compressed.</param>
    /// <param name="flush">The flush parameter. Should be set to <see cref="ZNoFlush" />, <see cref="ZSyncFlush" />, <see cref="ZPartialFlush" />, <see cref="ZBlock" />, <see cref="ZFullFlush" />, or <see cref="ZFinish" />.</param>
    /// <returns><see cref="ZOk"/> if some progress has been made (more input processed or more output produced), <see cref="ZStreamEnd" /> if all input has been consumed and all output has been produced (only when <param ref="flush" /> is set to <see cref="ZFinish" />), <see cref="ZStreamError" /> if the stream state was inconsistent (for example if <see cref="ZStream.Input"/> or <see cref="ZStream.Output"/> of <paramref name="strm"/> was not set or the state was inadvertently written over by the application), or <see cref="ZBufError" /> if no progress is possible (for example <see cref="ZStream.AvailableIn"/> or <see cref="ZStream.AvailableOut"/> of <paramref name="strm"/> was zero).</returns>
    ...
    /// <returns><see cref="ZOk"/> if success, <see cref="ZStreamError"/> if the stream state was inconsistent, or <see cref="ZDataError"/> if some input or output was discarded.</returns>
    ...
    /// <returns><see cref="ZOk"/> on success, <see cref="ZMemError"/> if there was not enough memory, or <see cref="ZStreamError"/> if the parameters are invalid.</returns>
    ...
    /// <para>Before calling this method, the application should ensure that at least one of the actions is possible, by providing more input and/or consuming more output, and updating the Next* and Available* properties accordingly. If the caller of the method does not provide both available input and available output space, it is possible that there will be no progress made. The application can consume the uncompressed output when it wants, for example when the output buffer is full (the <see cref="ZStream.AvailableOut"/> property of <paramref name="strm"/> then returns 0), or after each call of this method. If the method returns <see cref="ZOk"/> and the <see cref="ZStream.AvailableOut"/> property of <paramref name="strm"/> returns 0, it must be called again after making room in the output buffer because there might be more output pending.</para>
    /// <para>A <paramref name="flush"/> parameter value of <see cref="ZSyncFlush" /> requests the method to flush as much output as possible to the output buffer. <see cref="ZBlock" /> requests the method to stop if and when it gets to the next deflate block boundary. This will cause it to return immediately after the header and before the first block. The <see cref="ZTrees" /> option behaves as <see cref="ZBlock" /> does, but it also returns when the end of each deflate block header is reached, before any actual data in that block is decoded.</para>
    /// <para>The method should normally be called until it returns <see cref="ZStreamEnd" /> or an error. However if all decompression is to be performed in a single step (a single call of the method), the <paramref name="flush"/> parameter should be set to <see cref="ZFinish" />. In this case all pending input is processed and all pending output is flushed; the <see cref="ZStream.AvailableOut" /> property of <paramref name="strm"/> must be large enough to hold all of the uncompressed data for the operation to complete (the size of the uncompressed data may have been saved by the compressor for this purpose). The use of <see cref="ZFinish" /> is not required to perform an inflation in one step. However it may be used to inform the method that a faster approach can be used for the single method call.</para>
    /// </summary>
    /// <param name="strm">The stream to be decompressed.</param>
    /// <param name="flush">The flush parameter. Should be set to <see cref="ZNoFlush" />, <see cref="ZSyncFlush" />, <see cref="ZFinish" />, <see cref="ZBlock" />, or <see cref="ZTrees" />.</param>
    /// <returns><see cref="ZOk"/> if some progress has been made (more input processed or more output produced), <see cref="ZStreamEnd" /> if the end of the compressed data has been reached and all uncompressed output has been produced, <see cref="ZNeedDict" /> if a preset dictionary is needed at this point, <see cref="ZDataError" /> if the input data was corrupted (input stream not conforming to the zlib format or incorrect check value, in which case the <see cref="ZStream.Message"/> property of the <paramref name="strm"/> returns a <see cref="string" /> with a more specific error), <see cref="ZStreamError" /> if the stream structure was inconsistent (for example if <see cref="ZStream.Input"/> or <see cref="ZStream.Output"/> of <paramref name="strm"/> was not set, or the state was inadvertently written over by the application), <see cref="ZMemError" /> if there was not enough memory, <see cref="ZBufError" /> if no progress was possible or if there was not enough room in the output buffer when <see cref="ZFinish" /> is used.</returns>
    ...
    /// <param name="level">The compression level. It must be <see cref="ZDefaultCompression" />, or between 0 and 9: 1 gives best speed, 9 gives best compression, 0 gives no compression at all (the input data is simply copied a block at a time). <see cref="ZDefaultCompression" /> requests a default compromise between speed and compression (equivalent to level 6).</param>
    /// <param name="strategy">The strategy. Used to tune the compression algorithm. Use the value <see cref="ZDefaultStrategy" /> for normal data, <see cref="ZFiltered" /> for data produced by a filter (or predictor), <see cref="ZHuffmanOnly" /> to force Huffman encoding only (no string match), or <see cref="ZRle" /> to limit match distances to one (run-length encoding).</param>
    /// <returns><see cref="ZOk"/> on success, <see cref="ZStreamError"/> if the source stream state was inconsistent or if a parameter was invalid, or <see cref="ZBufError"/> if there was not enough output space to complete the compression of the available input data before a change in the strategy or approach.</returns>
    ...
    /// <param name="strm">An initialized compression stream.</param>
    /// <param name="dictionary">The compression dictionary.</param>
    /// <returns><see cref="ZOk"/> if success, or <see cref="ZStreamError"/> if the stream state is inconsistent (for example if <see cref="Deflate(ref ZStream, int)"/> has already been called for this stream or if not at a block boundary for raw deflate).</returns>
    ...
    /// <param name="strm">An initialized decompression stream.</param>
    /// <param name="dictionary">The decompression dictionary.</param>
    /// <returns><see cref="ZOk"/> if success, <see cref="ZStreamError"/> if a parameter is invalid (e.g. the stream state is inconsistent), <see cref="ZDataError"/> if the given dictionary doesn't match the expected one (incorrect Adler-32 value).</returns>
    /// <remarks>This method does not perform any decompression: this will be done by subsequent calls of <see cref="Inflate(ref ZStream, int)"/>.</remarks>
    public int InflateSetDictionary(ref ZStream strm, ReadOnlySpan<byte> dictionary)
    {
        return Inflater.InflateSetDictionary(ref strm, ref MemoryMarshal.GetReference(dictionary), (uint)dictionary.Length);
    }
    ...
    /// <returns><see cref="ZOk"/> if a possible full flush point has been found, <see cref="ZBufError"/> if no more input was provided, <see cref="ZDataError"/> if no flush point has been found, or <see cref="ZStreamError"/> if the stream structure was inconsistent.</returns>
    ...
    /// <returns><see cref="ZOk"/> if success, <see cref="ZMemError"/> if there was not enough memory, <see cref="ZStreamError"/> if the <paramref name="source"/> stream state was inconsistent. <see cref="ZStream.Message"/> is left unchanged in both <paramref name="source"/> and <paramref name="dest"/>.</returns>
    ...
    /// <returns><see cref="ZOk"/> if success, or <see cref="ZStreamError"/> if the source stream state was inconsistent, or if the <paramref name="windowBits"/> parameter is invalid.</returns>
    ...
    /// <returns><see cref="ZOk"/> if success, <see cref="ZMemError"/> if there was not enough memory, <see cref="ZBufError"/> if there was not enough room in the output buffer, or Z_STREAM_ERROR if any of the level parameters are invalid.</returns>
    /// <remarks><see cref="Compress(byte[], out int, byte[], int)"/> is equivalent to <see cref="Compress(byte[], out int, byte[], int, int)"/> with a level parameter of <see cref="ZDefaultCompression"/>.</remarks>
    ...
    /// <param name="source">The source buffer.</param>
    /// <returns><see cref="ZOk"/> if success, <see cref="ZMemError"/> if there was not enough memory, <see cref="ZBufError"/> if there was not enough room in the output buffer, or Z_STREAM_ERROR if any of the level parameters are invalid.</returns>
    /// <remarks><see cref="Compress(Span{byte}, out int, ReadOnlySpan{byte})"/> is equivalent to <see cref="Compress(Span{byte}, out int, ReadOnlySpan{byte}, int)"/> with a level parameter of <see cref="ZDefaultCompression"/>.</remarks>
    ...
    /// <returns><see cref="ZOk"/> if success, <see cref="ZMemError"/> if there was not enough memory, <see cref="ZBufError"/> if there was not enough room in the output buffer, or <see cref="ZDataError"/> if the input data was corrupted or incomplete or if <paramref name="sourceLen"/> is negative or greater than the length of <paramref name="source"/>.</returns>
    ...
    /// <returns><see cref="ZOk"/> if success, <see cref="ZMemError"/> if there was not enough memory, <see cref="ZBufError"/> if there was not enough room in the output buffer, or <see cref="ZDataError"/> if the input data was corrupted or incomplete.</returns>
    /// <remarks>In the case where there is not enough room, the method will fill the destination buffer with the uncompressed data up to that point.</remarks>

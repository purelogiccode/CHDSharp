...
internal static class Tree
{
    ...
    ///     encoded block.
    /// </summary>
    internal static void FlushBlock(
        ref ZStream strm,
        ref byte buf,
        uint storedLen,
        uint last,
    ...
    )
    {
        var s = strm.DeflateState;
        uint optLenb,
            staticLenb; // opt_len and static_len in bytes
        ...
        else
        {
            ...
            SendAllTrees(
                s,
                (uint)(s.LDesc.MaxCode + 1),
                (uint)(s.DDesc.MaxCode + 1),
                maxBlindex + 1,
                ref pendingBuf,
                ref dynLtree,
                ref dynDtree,
    ...
    /// </summary>
    internal static void StoredBlock(
        DeflateState s,
        ref byte buf,
        uint storedLen,
        uint last,
    ...
    )
    {
        ...
        if (!netUnsafe.IsNullRef(ref buf) && storedLen != 0)
            netUnsafe.CopyBlockUnaligned(
                ref Unsafe.Add(ref pendingBuf, s.Pending),
                ref buf,
                storedLen
            );
        s.Pending += storedLen;
    ...
    private static int DetectDataType(ref TreeNode dynLtree)
    {
        ...
        if (
            Unsafe.Add(ref dynLtree, 9U).fc != 0
            || Unsafe.Add(ref dynLtree, 10U).fc != 0
    ...
    )
    {
        ...
        do
        {
            ...
            Unsafe.Add(ref tree, node).fc = (ushort)(
                Unsafe.Add(ref tree, n).fc + Unsafe.Add(ref tree, m).fc
            );
    ...
    )
    {
        ...
        while (j <= s.HeapLen)
        {
            ...
            if (
                ...
                && Smaller(
                    ref tree,
                    (uint)Unsafe.Add(ref heap, j + 1),
                    (uint)Unsafe.Add(ref heap, j),
                    ref depth
                )
            )
                j++;
    ...
    )
    {
        ...
        netUnsafe.InitBlock(
            ref netUnsafe.As<ushort, byte>(ref blCount),
            0,
            MaxBits * sizeof(ushort)
        );
    ...
    private static void GenCodes(ref TreeNode tree, int maxCode, ref ushort blCount)
    {
         ...
         */
        Debug.Assert(
            code + Unsafe.Add(ref blCount, (uint)MaxBits) - 1 == (1 << MaxBits) - 1,
    ...
    ///     send.
    /// </summary>
    private static uint BuildBlTree(
        DeflateState s,
        ref TreeNode dynLtree,
        ref TreeNode dynDtree,
        ref TreeNode blTree,
    ...
    )
    {
        ...
        BuildTree(
            ...
            s.BlDesc,
            ref blTree,
            ref netUnsafe.NullRef<TreeNode>(),
            ref extra,
            ref blCount,
            ref heap,
            ref depth
    ...
    {
        return dist < 256
            ? Unsafe.Add(ref distCode, dist)
            : Unsafe.Add(ref distCode, 256 + (dist >> 7));
    }
    ...
    /// </summary>
    private static void SendAllTrees(
        DeflateState s,
        uint lcodes,
        uint dcodes,
        uint blcodes,
